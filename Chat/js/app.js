...
function myjsapp(peerClient) {
    ...
    return {
        ...
        createChatWindow: function(id) {
            ...
            $('.panel-body', panel).append('<span class="text-primary">Poti Sa Vorbesti</span>').append(history)
        ...
        showVideoCall : function (options) {
            ...
            if(options['video'])
                $('#videoCallPanel .title').text('Apel Video')
            else
                $('#videoCallPanel .title').text('Apel Voice')
        },
        showIncomingCall : function (peerId, options) {
            $('#callConfirmationModal').modal('show')
            if(options['video'])
                var txt = "Apel Video De La : " + peerId
            else
                var txt = "Apel Audio De La : " + peerId
        ...
        updateOnlieUsers : function (users) {
            ...
            list.empty()
            if(users.length == 0) {
                var usr = '<li>Aparent Nimeni Nu Este Online. Trist Pentru Tine.</li>'
                list.append(usr);
                return
            }
